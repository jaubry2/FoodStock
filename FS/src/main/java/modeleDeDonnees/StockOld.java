 ...
 */
 /*   public void ajouterAliment(String nomStock ,Aliment aliment, float quantite,LocalDate datePeremption) {
   ...
   /* public static Aliment getAlimentByName(List<Aliment> listAliments, String nom) {
     ...
     * @return l'objet Aliments correspondant au nom de l'aliment recherché, ou null si l'aliment n'est pas trouvé
     */
  /*  public  boolean  IsAlimentStockesByName(String nom, String nomStock) {
        for (AlimentStockes alimentstockes : stocks.getListFromMap(nomStock)) {
            if (alimentstockes.getAliment().getNom().equals(nom)) {
                return true;
            }
        }
        return false;
    }
    public  AlimentStockes GetAlimentStockesByName( String nom, String nomStock) {
    	List<AlimentStockes> stock = stocks.getListFromMap(nomStock);
